...
const VAPID_PUBLIC  = process.env.VAPID_PUBLIC_KEY  || "BH-yf_3iBj3GT7Dbys3RaIDgaB-cRZ5flDkWGG8jjxmpg8ZfoRbNZl1et72lfId_lMRB0roNK5zAjv3lV50aKjE";
const VAPID_PRIVATE = process.env.VAPID_PRIVATE_KEY || "2GCn32Re4x5LnJvQYzfyean1WX3csJ3pK3acWqmIMsI";

webpush.setVapidDetails("mailto:rodrigoprofissional07@gmail.com", VAPID_PUBLIC, VAPID_PRIVATE);
...
router.post("/subscribe", authenticate, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Erro ao salvar subscription: " + e.message });
  }
});
...
// Cron endpoint — protegido por secret header
router.post("/send-daily", async (req, res) => {
  const secret = req.headers["x-cron-secret"];
  if (secret !== (process.env.CRON_SECRET || "finovo2026push")) {
  ...
  try {
    ...
    const { rows: txRows } = await pool.query(`
      ...
      FROM transactions t
      WHERE t.date IN ($1, $2) AND t.status IN ('pendente', 'vencido')
    ...
    const { rows: subs } = await pool.query(
      `SELECT * FROM push_subscriptions WHERE organization_id = ANY($1)`, [orgIds]
    );
    ...
    for (const sub of subs) {
      ...
      if (!orgData) continue;
      const lines = [];
      if (parseInt(orgData.hoje_exp) > 0) lines.push(`💸 A pagar hoje: ${fmtBRL(orgData.hoje_val)} (${orgData.hoje_exp} lanç.)`);
      if (parseInt(orgData.amanha_exp) > 0) lines.push(`⏰ Vence amanhã: ${fmtBRL(orgData.amanha_val)} (${orgData.amanha_exp} lanç.)`);
      if (parseInt(orgData.hoje_inc) > 0) lines.push(`💰 A receber hoje: ${fmtBRL(orgData.hoje_inc_val)}`);
      if (lines.length === 0) continue;

      try {
        await webpush.sendNotification(
          { endpoint: sub.endpoint, keys: { p256dh: sub.p256dh, auth: sub.auth } },
          JSON.stringify({ title: "FINOVO — Resumo do dia", body: lines.join("\n"), url: "/", tag: "finovo-daily" })
        );
        sent++;
      } catch (e) {
